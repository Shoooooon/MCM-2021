...
#2021 data
dat2020 = read_csv("Archive Data/viirs-snpp_2020_Australia.csv" #, 
                   #col_types = cols(acq_date = col_date(format = "%Y-%m-%d")),
                   )
dat2020$acq_date = as.Date(dat2020$acq_date,format = "%m/%d/%Y")
dat_all2 = rbind(dat_all, dat2020)

#-----------------------------------------------------------------
#Make csv file: 
#1) Select Victoria
dat_victoria = dat_all2 %>% filter((latitude < -36&latitude >= -39& longitude <142& longitude >= 141)|
